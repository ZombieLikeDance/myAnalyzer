...
class Maze(object):
    ...
    def set_reward(self, reward=None):
        ...
        """
        curr_dist = np.linalg.norm(np.array(self.robot['loc'])  - np.array(self.destination))
        curr_dist = curr_dist / np.linalg.norm(np.array(self.start_point)  - np.array(self.destination))
        if reward is None:
            self.reward = {
                "hit_wall": -10.,
                "destination": 50.,
                "default": -0.1,
                #"default":round(-curr_dist/10,4)
            }
        else:
            self.reward = reward
